...
function Invoke-Matrix {
  ...
  $reg = Invoke-Api -Method 'POST' -Url "$AuthBase/register" -Body @{ fullName='Smoke User'; email=$customerEmail; password=$customerPassword }
  $results['Auth register (customer)'] = $reg.Status

  $adminLogin = Invoke-Api -Method 'POST' -Url "$AuthBase/login" -Body @{ email='admin@smartsure.com'; password='Admin@123' }
  ...
  $customerToken = $null
  $custLogin = Invoke-Api -Method 'POST' -Url "$AuthBase/login" -Body @{ email='john@example.com'; password='Customer@123' }
  if ($custLogin.Content) {
    try { $customerToken = ($custLogin.Content | ConvertFrom-Json).token } catch {}
  }

  if (-not $customerToken) {
    $custLoginFallback = Invoke-Api -Method 'POST' -Url "$AuthBase/login" -Body @{ email=$customerEmail; password=$customerPassword }
  ...
  $profileResp = Invoke-Api -Method 'GET' -Url "$AuthBase/profile" -Headers $customerHeaders
  $results['Identity profile (customer)'] = $profileResp.Status

  $idUsers = Invoke-Api -Method 'GET' -Url "$AuthBase/admin/users" -Headers $adminHeaders
  $results['Identity admin users list'] = $idUsers.Status

  $idCount = Invoke-Api -Method 'GET' -Url "$AuthBase/admin/users/count" -Headers $adminHeaders
  ...
  $idDisable = Invoke-Api -Method 'PUT' -Url "$AuthBase/admin/users/$targetUserId/status" -Headers $adminHeaders -Body @{ isActive=$false }
  $results['Identity admin update user status (disable)'] = $idDisable.Status
  $idEnable = Invoke-Api -Method 'PUT' -Url "$AuthBase/admin/users/$targetUserId/status" -Headers $adminHeaders -Body @{ isActive=$true }
  $results['Identity admin update user status (enable)'] = $idEnable.Status

  $types = Invoke-Api -Method 'GET' -Url "$PolicyBase/types" -Headers $customerHeaders
  ...
  $premium = Invoke-Api -Method 'POST' -Url "$PolicyBase/calculate-premium" -Headers $customerHeaders -Body @{ policyTypeId=$policyTypeId; age=30; startDate=$start; endDate=$end }
  $results['Policy premium calculate (customer)'] = $premium.Status

  $createPolicy = Invoke-Api -Method 'POST' -Url "$PolicyBase" -Headers $customerHeaders -Body @{ policyTypeId=$policyTypeId; age=30; startDate=$start; endDate=$end }
  $results['Policy create (customer)'] = $createPolicy.Status

  $myPolicies = Invoke-Api -Method 'GET' -Url "$PolicyBase/my" -Headers $customerHeaders
  ...
  $createClaim = Invoke-Api -Method 'POST' -Url "$ClaimsBase" -Headers $customerHeaders -Body @{ policyId=$policyId; incidentDate=(Get-Date).Date.AddDays(-1).ToString('o'); description='Smoke claim' }
  ...
  $submitClaim = Invoke-Api -Method 'POST' -Url "$ClaimsBase/$claimId/submit" -Headers $customerHeaders -Body @{}
  $results['Claim submit (customer)'] = $submitClaim.Status

  $claimsList = Invoke-Api -Method 'GET' -Url "$ClaimsBase" -Headers $adminHeaders
  $results['Claims admin list'] = $claimsList.Status

  $claimUpdate = Invoke-Api -Method 'PUT' -Url "$ClaimsBase/$claimId/status" -Headers $adminHeaders -Body @{ status='UnderReview'; adminNote='Smoke review' }
  $results['Claim status update (admin)'] = $claimUpdate.Status

  $admDash = Invoke-Api -Method 'GET' -Url "$AdminBase/dashboard" -Headers $adminHeaders
  $results['Admin dashboard'] = $admDash.Status

  $admClaims = Invoke-Api -Method 'GET' -Url "$AdminBase/claims" -Headers $adminHeaders
  $results['Admin claims list'] = $admClaims.Status

  $admUsers = Invoke-Api -Method 'GET' -Url "$AdminBase/users" -Headers $adminHeaders
  $results['Admin users list'] = $admUsers.Status

  $admDisable = Invoke-Api -Method 'PUT' -Url "$AdminBase/users/$targetUserId/status" -Headers $adminHeaders -Body @{ isActive=$false }
  $results['Admin user status update (disable)'] = $admDisable.Status
  $admEnable = Invoke-Api -Method 'PUT' -Url "$AdminBase/users/$targetUserId/status" -Headers $adminHeaders -Body @{ isActive=$true }
  $results['Admin user status update (enable)'] = $admEnable.Status

  $admTypes = Invoke-Api -Method 'GET' -Url "$PolicyBase/admin/types" -Headers $adminHeaders
  $results['Admin policy types list'] = $admTypes.Status

  $typeName = "SmokeType-$Mode-$stamp"
  $admCreateType = Invoke-Api -Method 'POST' -Url "$PolicyBase/admin/types" -Headers $adminHeaders -Body @{ name=$typeName; description='Smoke policy type'; baseAmount=1234 }
  ...
  $admUpdateType = Invoke-Api -Method 'PUT' -Url "$PolicyBase/admin/types/$newTypeId" -Headers $adminHeaders -Body @{ name="$typeName-upd"; description='Updated'; baseAmount=1500 }
  $results['Admin policy type update'] = $admUpdateType.Status

  $admDeleteType = Invoke-Api -Method 'DELETE' -Url "$PolicyBase/admin/types/$newTypeId" -Headers $adminHeaders
...
$gw = Invoke-Matrix -Mode 'gateway' -AuthBase 'http://localhost:5000/gateway/auth' -PolicyBase 'http://localhost:5000/gateway/policies' -ClaimsBase 'http://localhost:5000/gateway/claims' -AdminBase 'http://localhost:5000/gateway/admin'
$dr = Invoke-Matrix -Mode 'direct' -AuthBase 'http://localhost:5265/api/auth' -PolicyBase 'http://localhost:5145/api/policies' -ClaimsBase 'http://localhost:5084/api/claims' -AdminBase 'http://localhost:5073/api/admin'

function Get-StatusText([int]$s){ if($s -ge 200 -and $s -lt 300){"$s (PASS)"} else {"$s (FAIL)"} }
...
foreach($label in $labels){
  $gs=[int]$gw[$label]; $ds=[int]$dr[$label]
  $lines.Add("$label|$(Get-StatusText $gs)|$(Get-StatusText $ds)")
