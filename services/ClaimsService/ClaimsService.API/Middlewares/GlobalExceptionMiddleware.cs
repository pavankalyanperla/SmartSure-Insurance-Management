...
namespace ClaimsService.API.Middlewares;

/// <summary>
/// Global exception handling middleware for ClaimsService.
...
public class GlobalExceptionMiddleware
{
    ...
    private async Task HandleExceptionAsync(HttpContext context, Exception exception)
    {
        var (statusCode, message) = exception switch
        {
            ...
            ClaimException claimEx
                => (claimEx.StatusCode, claimEx.Message),
        ...
        if (exception is ClaimException)
            _logger.LogWarning("Claims domain exception [{Type}]: {Message}", exception.GetType().Name, exception.Message);
        else
        ...
        var response = new ErrorResponse
        {
            ...
            Message    = message,
            Detail     = _env.IsDevelopment() && exception is not ClaimException
                             ? exception.ToString()
                             : null
        };
